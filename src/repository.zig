...
fn writeSkill(c: *Context, d: Discovered, created_root_out: *bool) !types.RepositoryBatchImport {
    ...
    try recording_copy.copyTree(c.arena, c.io, src, dst, skill_dir, "", recording_copy.excludeGit(), snk.sink());
...
/// Adapts `recording_copy.copyTree`'s `Sink` to a repository import action list:
/// each copied regular file appends a `copy_file` action with its absolute
/// destination path. The emitted path is arena-owned by copyTree, so it is
/// appended directly. `.git` exclusion is handled by `excludeGit()`.
